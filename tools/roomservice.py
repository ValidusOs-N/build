...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from Validus Github (http://github.com/ValidusOs-N)." % device

repositories = []

page = 1
while not depsonly:
    request = Request("https://api.github.com/users/ValidusOs-N/repos?page=%d" % page)
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'ValidusOs-N/%s already exists' % (repo_name)
            else:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": repo_name, "revision": "7.0" })
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for ValidusOs-N/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: ValidusOs-N/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "ValidusOs-N/%s" % repo_name, "revision": "7.0" })
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("android_device_") and repo_name.endswith("_" + device):
            ...
            add_to_manifest([{'repository':repo_name,'target_path':repo_path,'branch':'7.0'}])
...
print "Repository for %s not found in the Validus Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/validus_manifest.xml" % device
